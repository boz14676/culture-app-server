...
namespace App\Services\Alipay\v1;
...
class AlipayNotify
{
    ...
    public function verifyNotify()
    {
        if (empty($_POST)) {//判断POST来的数组是否为空
            ...
            Log::debug('回调校验:' . (int)$isSign);
            Log::info('支付post:'. json_encode($_POST));
    ...
    public function getSignVeryfy($para_temp, $sign)
    {
        ...
        switch (strtoupper(trim($this->alipay_config['sign_type']))) {
            case "RSA":
                ...
                Log::debug("支付调试：", ['prestr' => $prestr, 'sign' => $sign, 'public_key' => trim($this->alipay_config['alipay_public_key'])]);
                break;
            default:
                $isSgin = false;
        }
